...
let soundEffect;
let showNull = false; // Flag to display the word "null"
...
function setup() {
  ...
  for (let i = 0; i < sentence.length; i++) {
    ...
    // Adjust spacing for inner letters
    if (distance < swirlRadius / 2) {
      distance *= 1.2; // Increase spacing as letters get closer to the center
    }
...
function draw() {
  ...
  // Display "null" at the center after the explosion
  if (showNull) {
    fill(255); // White text
    textSize(64); // Larger font size for "null"
    textAlign(CENTER, CENTER);
    text("null", swirlCenter.x, swirlCenter.y); // Center the word "null"
  }
}
...
function mousePressed() {
  if (ellipseAnimating && ellipseSize > swirlRadius * 2.4) {
    ...
    if (distanceFromCenter < ellipseSize / 2) {
      ...
      // Trigger display of "null" after explosion
      setTimeout(() => {
        showNull = true; // Enable display of "null"
      }, 1000); // Delay to ensure particles disperse before "null" appears
    }
  }
}
...
class Interact {
  ...
  show() {
    noStroke();
    let baseFontSize = 24; // Default font size
    let shrinkPhrase = "just a long pointless sentence moving in a circle.";
    let shrinkStartIndex = sentence.indexOf(shrinkPhrase); // Starting index of the phrase

    // Gradually reduce font size for the specified part
    if (shrinkStartIndex !== -1 && this.char !== ' ') {
      let charIndex = sentence.indexOf(this.char);
      if (charIndex >= shrinkStartIndex) {
        let shrinkFactor = map(charIndex, shrinkStartIndex, sentence.length, 1, 0.5); // Reduce size
        baseFontSize *= shrinkFactor;
      }
    }

    fill(255); // White text
    textSize(baseFontSize); // Dynamically set the font size
    push();
